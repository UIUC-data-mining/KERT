...
def load_file(filename):
    ...
    f = open(filename, 'r')
    for line in f:
        doc = re.findall(r'[a-zA-Z0-9]+',line.lower())
        if len(doc)>0:
            tids.append(int(doc[1]))
            pids.append(int(doc[0]))
    f.close()
    return (pids,tids)

def load_file_paper(filename):
    corpus = []
    idx=[]
    f = open(filename, 'r')
    for line in f:
...
class Vocabulary:
    ...
    def PT_to_idlist(self, pids, tids):
        num_papers = pids[len(pids)-1]
        final_list = [[] for x in xrange(0,num_papers + 1)]
        for i in range(len(pids)):
            final_list[pids[i]].append(tids[i])
        return final_list
